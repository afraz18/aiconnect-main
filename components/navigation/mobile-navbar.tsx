...
const MobileNavbar = () => {
    ...
    // Removed: const { isSignedIn, signOut } = useAuth();
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
                ...
                <SheetContent className="w-screen">
                    <SheetClose asChild className="absolute top-3 right-5 bg-background z-20 flex items-center justify-center">
                        {/* Removed text-neutral-600 to rely on component theme, 
                            which should be text-foreground or muted by default for a button. */}
                        <Button size="icon" variant="ghost">
                            <X className="w-5 h-5" />
                        </Button>
                    </SheetClose>
                    <div className="flex flex-col items-start w-full py-2 mt-10">
                        <div className="flex items-center justify-evenly w-full space-x-2">
                            {/* Removed conditional rendering (Clerk) - now always shows Sign In/Up */}
                            <>
                                <Link
                                    href="/auth/sign-in"
